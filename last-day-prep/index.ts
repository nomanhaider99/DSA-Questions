...
function frequncyOfEachChar(str: string): Record<string, number> {
...
function rotateAtK(nums: number[], k: number): number[] {
...
function checkPowerTwo(n: number): boolean {
    let result = false;
    for (let x = 1; x <= n; x++) {
        if (x ** 2 == n) {
            console.log(x ** 2)
...
function perfectSquare (n: number): number {
    let output = 0;
    let left = 0;
    let right = n;
